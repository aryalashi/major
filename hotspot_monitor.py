...
class HotspotMonitor:
    ...
    # ────────────────────────────────────────────────────────────────────────
    # ADVANCED SEARCH & ATTACK TRACKING
    # ────────────────────────────────────────────────────────────────────────
    ...
    def search_devices(self, query: str) -> List[Dict]:
        ...
        Args:
            query: Search query - can be:
                ...
                - IP address (e.g., "192.168.43.50")
                - Hostname/device name (e.g., "iPhone")
                - Partial match (e.g., "aaronphone")
        
        Returns:
            List of matching devices with details
        """
        results = []
        ...
        try:
            with self.clients_lock:
                for mac, client in self.clients.items():
                    # Check for matches
                    if (query_lower in mac.lower() or 
                        (client.ip and query_lower in client.ip.lower()) or
                        query_lower in client.hostname.lower()):
                        ...
                        results.append({
                            ...
                            'threat_level': client.threat_level,
                            'connected_at': client.connected_at.isoformat(),
                            'session_duration': client.get_session_duration(),
                            'data_usage': client.get_data_usage(),
            ...
            logger.error(f"[SEARCH] Error searching devices: {e}")
            return []
    
    def scan_all_devices(self) -> List[Dict]:
        """Scan and list all connected devices with full details"""
        try:
            with self.clients_lock:
                devices = []
                for mac, client in self.clients.items():
                    devices.append({
                        ...
                        'attacks_from': len(client.attacks_from_device),
                        'attacks_to': len(client.attacks_to_device),
                        'total_attacks': (len(client.attacks_from_device) + 
                                        len(client.attacks_to_device)),
                    })
                
                return sorted(devices, key=lambda x: x['total_attacks'], reverse=True)
        
        except Exception as e:
            logger.error(f"[SCAN] Error scanning devices: {e}")
            return []
    
    def get_attack_log(self, device_mac: str = None, attack_type: str = None) -> List[Dict]:
        """
        Get attack log filtered by device and/or attack type.
        
        Args:
            device_mac: Filter by specific device (None = all devices)
            attack_type: Filter by attack type (None = all types)
        
        Returns:
            List of attacks with details
        """
        attacks = []
        ...
        try:
            with self.clients_lock:
                for mac, client in self.clients.items():
                    ...
                    if device_mac and mac.upper() != device_mac.upper():
                        continue
                    
                    # Attacks FROM device
                    for attack in client.attacks_from_device:
                        if attack_type is None or attack_type.lower() in attack['type'].lower():
                            attacks.append({
                                'timestamp': attack['timestamp'],
                                'device_mac': mac,
                                'device_hostname': client.hostname,
                                'device_ip': client.ip,
                                'direction': 'FROM_DEVICE',
                                'attack_type': attack['type'],
                                'target_ip': attack.get('target_ip'),
                                'target_port': attack.get('target_port'),
                                'confidence': attack.get('confidence'),
                                'severity': attack.get('severity'),
                            })
            ...
            return sorted(attacks, key=lambda x: x['timestamp'], reverse=True)
        
        except Exception as e:
            logger.error(f"[ATTACK LOG] Error retrieving attack log: {e}")
            return []
    
    def get_device_attack_stats(self, device_mac: str) -> Dict:
        """Get attack statistics for a specific device"""
        try:
            client = self.get_client(device_mac)
            if not client:
                return {'error': f'Device {device_mac} not found'}
            
            attacks_from = client.attacks_from_device
            attacks_to = client.attacks_to_device
            
            # Group by attack type
            attack_types_from = defaultdict(int)
            attack_types_to = defaultdict(int)
            
            for attack in attacks_from:
                attack_types_from[attack['type']] += 1
            
            for attack in attacks_to:
                attack_types_to[attack['type']] += 1
            
            return {
                'device_mac': device_mac,
                'device_hostname': client.hostname,
                'device_ip': client.ip,
                'total_attacks_from': len(attacks_from),
                'total_attacks_to': len(attacks_to),
                'total_attacks': len(attacks_from) + len(attacks_to),
                'threat_level': client.threat_level,
                'is_blocked': client.is_blocked,
                'attack_types_from': dict(attack_types_from),
                'attack_types_to': dict(attack_types_to),
                'recent_attacks': {
                    'from_device': attacks_from[-5:] if attacks_from else [],
                    'to_device': attacks_to[-5:] if attacks_to else [],
                },
            }
        
        except Exception as e:
            logger.error(f"[DEVICE STATS] Error getting device stats: {e}")
            return {'error': str(e)}
    
    def get_attack_patterns(self) -> Dict:
        """Analyze attack patterns across all devices"""
        try:
            patterns = {
                'total_attacks': 0,
                'attack_types': defaultdict(int),
                'most_attacked_device': None,
                'most_attacking_device': None,
            ...
            with self.clients_lock:
                for mac, client in self.clients.items():
                    # Count attacks from device
                    if client.attacks_from_device:
                        if patterns['most_attacking_device'] is None:
                            patterns['most_attacking_device'] = {
                                'hostname': client.hostname,
                                'count': len(client.attacks_from_device)
                            }
                        elif len(client.attacks_from_device) > patterns['most_attacking_device']['count']:
                            patterns['most_attacking_device'] = {
                                'hostname': client.hostname,
                                'count': len(client.attacks_from_device)
                            }
                        ...
                        for attack in client.attacks_from_device:
                            patterns['total_attacks'] += 1
                            patterns['attack_types'][attack['type']] += 1
                            patterns['top_targets'][attack.get('target_ip', 'Unknown')] += 1
                            patterns['severity_distribution'][attack.get('severity', 'MEDIUM')] += 1
                    
                    # Count attacks to device
                    if client.attacks_to_device:
                        if patterns['most_attacked_device'] is None:
                            patterns['most_attacked_device'] = {
                                'hostname': client.hostname,
                                'count': len(client.attacks_to_device)
                            }
                        elif len(client.attacks_to_device) > patterns['most_attacked_device']['count']:
                            patterns['most_attacked_device'] = {
                                'hostname': client.hostname,
                                'count': len(client.attacks_to_device)
                            }
                        ...
                        for attack in client.attacks_to_device:
                            patterns['total_attacks'] += 1
                            patterns['attack_types'][attack['type']] += 1
                            patterns['top_sources'][attack.get('source_ip', 'Unknown')] += 1
                            patterns['severity_distribution'][attack.get('severity', 'MEDIUM')] += 1
            
            # Convert defaultdicts to regular dicts
            patterns['attack_types'] = dict(patterns['attack_types'])
            patterns['top_targets'] = dict(sorted(patterns['top_targets'].items(),
                                                  key=lambda x: x[1], reverse=True)[:10])
            patterns['top_sources'] = dict(sorted(patterns['top_sources'].items(),
                                                  key=lambda x: x[1], reverse=True)[:10])
            ...
            logger.error(f"[PATTERNS] Error analyzing patterns: {e}")
            return {}
