...
class NIDSMainWindow(QMainWindow):
    ...
    def _on_alert(self, alert: dict):
        ...
        # Schedule batch processing if not already scheduled (200ms delay reduces freezing)
        if not self._update_pending:
            self._update_pending = True
            QTimer.singleShot(200, self._process_pending_alerts)
    
    def _process_pending_alerts(self):
        """Process all queued alerts in a batch."""
        if not self._pending_alerts:
            self._update_pending = False
            return
        
        # Cap batch size to prevent massive freezes (process max 50 per batch)
        alerts_to_process = self._pending_alerts[:50]
        self._pending_alerts = self._pending_alerts[50:]
        
        # Disable ALL updates during batch
        self.alert_table.setUpdatesEnabled(False)
        self.evidence_log.setUpdatesEnabled(False)
        
        # Batch collect all updates, apply them once at the end
        rules_to_update = {}  # rule -> sev, for batch sidebar update
        ...
        for alert in alerts_to_process:
            ...
            # Track rule for batch update (avoid 100+ sidebar updates)
            rules_to_update[rule] = sev
            
            # Track last alert (only update once at end)
            last_alert_data = (ts, sev, rule, alert)
            ...
            # Batch evidence log (don't append every time)
        ...
        # Re-enable updates
        self.alert_table.setUpdatesEnabled(True)
        self.evidence_log.setUpdatesEnabled(True)
        
        # Now do batch updates ONCE instead of 100+ times
        if rules_to_update:
            for rule, sev in rules_to_update.items():
                self._update_rule_counter(rule, sev)
        
        # Update last alert ONCE
        if last_alert_data:
            ts, sev, rule, alert = last_alert_data
            self.last_alert_lbl.setText(
                f"{ts}\n{sev}  {rule}\n"
        ...
        # Append all evidence lines at once (batch append)
        if evidence_lines:
            self.evidence_log.appendPlainText("\n".join(evidence_lines))
        
        # Scroll and update result label
        self.alert_table.scrollToBottom()
        total = len(self._all_alerts)
        vis   = self.alert_table.rowCount()
        self.result_lbl.setText(f"SHOWING {vis} / {total}")
        
        # Schedule next batch if there are more alerts to process
        if self._pending_alerts:
            QTimer.singleShot(100, self._process_pending_alerts)
        else:
            self._update_pending = False
